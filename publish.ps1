<#
  ...
  RUN (from a terminal opened in this repo folder):
    ...
    Update + release v1.21.4:  powershell -ExecutionPolicy Bypass -File .\publish.ps1 -Release v1.21.4
    With a commit message:    powershell -ExecutionPolicy Bypass -File .\publish.ps1 -Release v1.21.4 -CommitMessage "Subject`n`nBody"

  REDO A RELEASE (the tag must not already exist on GitHub):
    gh release delete v1.21.4 --cleanup-tag --yes
    powershell -ExecutionPolicy Bypass -File .\publish.ps1 -Release v1.21.4 -CommitMessage "..."
#>
...
$WidgetName    = "basemap-gallery-custom"   # widget folder name (must match EB folder + repo subfolder)
$RepoName      = "basemap-gallery-custom-widget"
...
# "tests" is here for the same reason: two of the suites read src\exb-editor-shims.d.ts and
# type check the whole src folder, so they would fail out of the box in a zip that leaves the
# shims behind. The tests are in the GitHub repo, which is where they are meant to run.
# Paths are relative to the widget folder; wildcards allowed in the leaf name.
$ReleaseOnlyExclude = @(
...
try {
    ...
    if ($Release -ne "") {
        if (-not $gh) {
            ...
            # Stage a clean copy of the repo subfolder, strip the editor-only files, and zip
            # that. The repo copy itself is untouched, so the shims stay on GitHub.
